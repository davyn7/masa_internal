...
@router.get("/mrr_all_customers", tags=["Financial KPIs"])
async def get_mrr_all_customers():
    try:
        manager = KpiManager()
        return await manager.get_mrr_all_customers()
    except Exception as e:
        raise e
...
async def get_mrr_by_site(site_legal_name: str):
    try:
        manager = KpiManager()
        return await manager.get_mrr_by_site(site_legal_name)
    except Exception as e:
        raise e
...
@router.get("/arr_all_customers", tags=["Financial KPIs"])
async def get_arr_all_customers():
    try:
        manager = KpiManager()
...
async def get_arr_by_site(site_legal_name: str):
    try:
        manager = KpiManager()
        return await manager.get_arr_by_site(site_legal_name)
    except Exception as e:
        raise e
